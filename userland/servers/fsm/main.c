...
/* LAB 5 TODO BEGIN */
void gen_dispatch_ipc_msg(struct mount_point_info_node *mpinfo,
                          struct ipc_msg **ipc_msg_dispatch,
                          struct fs_request **fr_dispatch,
                          struct fs_request *fr)
{
        *ipc_msg_dispatch = ipc_create_msg(
                mpinfo->_fs_ipc_struct, sizeof(struct fs_request), 0);
        *fr_dispatch = (struct fs_request *)ipc_get_msg_data(*ipc_msg_dispatch);
        memcpy(*fr_dispatch, fr, sizeof(struct fs_request));
}
/* LAB 5 TODO END */
...
void fsm_server_dispatch(struct ipc_msg *ipc_msg, u64 client_badge)
{
        ...
        /* LAB 5 TODO BEGIN */
        struct ipc_msg *ipc_msg_dispatch = NULL;
        struct fs_request *fr_dispatch;
        /* LAB 5 TODO END */
        ...
        case FS_REQ_OPEN:
                ...
                gen_dispatch_ipc_msg(
                        mpinfo, &ipc_msg_dispatch, &fr_dispatch, fr);
        ...
        case FS_REQ_WRITE:
                ...
                break;
        case FS_REQ_UNLINK:
                mpinfo = get_mount_point(fr->unlink.pathname,
                                         strlen(fr->unlink.pathname));
                gen_dispatch_ipc_msg(
                        mpinfo, &ipc_msg_dispatch, &fr_dispatch, fr);
                strip_path(mpinfo, fr_dispatch->unlink.pathname);
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_dispatch);
                ipc_set_msg_data(
                        ipc_msg, ipc_get_msg_data(ipc_msg_dispatch), 0, ret);
                break;
        case FS_REQ_RMDIR:
                mpinfo = get_mount_point(fr->rmdir.pathname,
                                         strlen(fr->rmdir.pathname));
                gen_dispatch_ipc_msg(
                        mpinfo, &ipc_msg_dispatch, &fr_dispatch, fr);
                strip_path(mpinfo, fr_dispatch->rmdir.pathname);
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_dispatch);
                ipc_set_msg_data(
                        ipc_msg, ipc_get_msg_data(ipc_msg_dispatch), 0, ret);
                break;
        case FS_REQ_MKDIR:
                mpinfo = get_mount_point(fr->mkdir.pathname,
                                         strlen(fr->mkdir.pathname));
                gen_dispatch_ipc_msg(
                        mpinfo, &ipc_msg_dispatch, &fr_dispatch, fr);
                strip_path(mpinfo, fr_dispatch->mkdir.pathname);
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_dispatch);
                ipc_set_msg_data(
                        ipc_msg, ipc_get_msg_data(ipc_msg_dispatch), 0, ret);
                break;
        ...
        case FS_REQ_CREAT:
                ...
                strip_path(mpinfo, fr_dispatch->creat.pathname);
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_dispatch);
                ipc_set_msg_data(
                        ipc_msg, ipc_get_msg_data(ipc_msg_dispatch), 0, ret);
                break;
        case FS_REQ_GET_SIZE:
                ...
                gen_dispatch_ipc_msg(
                        mpinfo, &ipc_msg_dispatch, &fr_dispatch, fr);
                strip_path(mpinfo, fr_dispatch->getsize.pathname);
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_dispatch);
                ipc_set_msg_data(
                        ipc_msg, ipc_get_msg_data(ipc_msg_dispatch), 0, ret);
                break;
        ...
        if (ipc_msg_dispatch != NULL) {
                ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg_dispatch);
        }
