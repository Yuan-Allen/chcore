                       ...
                       size_t size)
{
        ...
        /* LAB 5 TODO BEGIN */
        to_write = size;
        u64 max_page_no = DIV_ROUND_UP(inode->size, PAGE_SIZE);
        for (size_t page_write_len = 0; to_write > 0;
             to_write -= page_write_len,
                    cur_off += page_write_len,
                    data += page_write_len) {
                ...
                page_write_len = to_write < (PAGE_SIZE - page_off) ?
                                         to_write :
                                         (PAGE_SIZE - page_off);
                      ...
                      size_t size)
{
        ...
        /* LAB 5 TODO BEGIN */
        to_read = offset + size > inode->size ? inode->size - offset : size;

        for (size_t page_read_len = 0; to_read > 0; to_read -= page_read_len,
                    cur_off += page_read_len,
                    buff += page_read_len) {
                page_no = cur_off / PAGE_SIZE;
                page_off = cur_off % PAGE_SIZE;
                page = radix_get(&inode->data, page_no * PAGE_SIZE);
                page_read_len = to_read < (PAGE_SIZE - page_off) ?
                                        to_read :
                                        (PAGE_SIZE - page_off);
...
int tfs_load_image(const char *start)
{
        ...
        for (f = g_files.head.next; f; f = f->next) {
                ...
                len = strlen(leaf);
                if (S_ISDIR(f->header.c_mode) && err == -ENOENT) {
                        if ((err = tfs_mkdir(dirat, leaf, len)) < 0)
                                return err;
                } else if (S_ISREG(f->header.c_mode)) {
                        if ((err == -ENOENT)
                            && (err = tfs_creat(dirat, leaf, len)) < 0)
                                return err;
